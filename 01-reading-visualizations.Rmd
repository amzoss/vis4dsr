...
source('style/load_style.R')

theme_set(theme_ch_1())
...
```{r bar-panel, fig.cap='Three sample bar charts. A: A sample bar chart with vertical bars. B: A sample bar chart with both positive and negative values. C: A sample bar chart, with the bars oriented horizonally.'}

vert_pos <- ggplot(
  inclusiveness_index %>% 
    mutate(Continent = Continent %>% as_factor() %>% fct_infreq()),
  mapping = aes(x = Continent)) +
  geom_bar(fill = "grey50", width = .75) +
...
```{r lolli-panel, fig.cap='Two sample lollipop plots. A: A sample lollipop plot. B: A sample lollipop plot with both positive and negative values.'}
...
```{r bar-color-panel, fig.cap='Three bar charts with color. A: A sample stacked bar chart. B: A sample filled stacked bar chart. C: A sample grouped bar chart.'}
...
(bar_stacked + bar_filled) / bar_grouped +
  plot_annotation(tag_levels = 'A') +
...
```{r dumbbell-panel, fig.cap='A: A grouped bar chart focusing on two continents. B: The same grouped bar chart with a different arrangement of the categorical variables. C: A dumbbell plot comparing two continents.'}

# TODO: color or black and white?

grouped_bar_pair <- ggplot(
...
dumbbell <- ggplot(
  ...
  geom_point(aes(color = Continent), size=6, show.legend = FALSE) +
  labs(y="Number of Records", x="Inclusiveness Index") +
  scale_color_manual(values=c(continent.colors["Asia"],continent.colors["Europe"]))

((grouped_bar_pair / grouped_bar_pair_pivot) | dumbbell) +
  plot_annotation(tag_levels = 'A') +
  plot_layout(guides = 'keep', widths = c(2,4))
...
jitter_1 <- jitter(x, factor = 50)

jitter_2 <- jitter(x, factor = 200)
...
combined <- bind_rows(pos.lin, jitter1, jitter2) %>%
  mutate(type = type %>% as_factor() %>% 
           fct_relevel(c("very diffuse", "somewhat diffuse",
                         "perfect correlation")))
...
A pie chart is typically used to explore the components that make up a total. This
is often called a "part-to-whole" relationship. In a line chart, the line helps
emphasize change over time by using a shape that has a strong connection to the
idea of motion or change (a line). With a pie chart, the shape is a circle divided
up into pie slices or wedges, and that shape has a strong connection to the idea of
comparison between a fraction and a total.

The kinds of data represented in a pie chart are the same as the kinds of data in
a bar chart - one categorical variable (the label) and one numerical variable 
(the size or proportion of the wedges). In the bar chart, the numerical variable
corresponds to the length of the bars, and negative bars can travel in a different
direction from the positive bars. In a pie chart, the numerical variable corresponds
to the area (or angle) of the pie wedge. For each value of the categorical variable, 
there is one pie wedge, and the percentage of the pie that the wedge takes up is the
same percentage you get if you compare the numerical value for that category to the
sum of the numerical values for all of the categories (i.e., the total sum for 
that variable). That's the same kind of calculation that happens when you have
a stacked bar chart that is also "filled" to 100%.

Because a pie chart shows the percentage of a total, it tends to be used for numerical
variables where the values are all positive and it makes sense to group all of the
data points into a total. Some good data to use with a pie chart might the viewers
of a TV show divided up by age groups, or the amount of money someone spends in a
month divided up by different kinds of purchases. Data that would not be as well
represented by a pie chart include the tax refund amount someone received for the
previous five years (because there isn't really a need to sum together five
years of tax refunds), or the average life expectancy of a group of countries 
(again, you wouldn't generally need to add together life expectancies). Likewise,
pie charts don't make sense for data where the numerical values can be negative -- 
it doesn't make sense to try to show a negative value as a percentage of a total.

When reading a pie chart, you will typically compare the sizes of the wedges to
each other, as well as to the whole pie chart. Comparing the area of two different
shapes can actually be a bit tricky. In a pie chart, each wedge is rotated at a
different angle. If all of the wedges were oriented the same way -- for example, 
they all had one edge starting at the "12:00" position on the pie opened 
up to the right --  we would only have to compare the tilt of the second edge of 
the wedge to see which wedge was larger. When comparing wedges that are all rotated
at different angles and aren't very close to each other, it can be very hard to
tell the difference between similarly sized wedges. Often a pie chart is not meant
to be read with great precision. The goal instead may be to show that one wedge
is much larger than the others, or that all of the wedges are approximately the same,
or that one wedge is much smaller than expected. The more wedges there are in the
pie chart, however, the more difficult these comparisons can be. A pie chart that
has a small number of wedges that are all varying sizes tends to be the easiest
to read.

If the wedges are connected to categories that have a natural order (for example,
age groups or days of the week), it works best to put the wedges in that order 
as they rotate around the circle. If there is no natural order, the pie chart
might organize the wedges by size, with the largest wedge in the most prominent 
position -- starting at the "12:00" position. That makes it easy to find the 
most influential data point and also rank the wedges by size.

You may see pie charts where every wedge is a different color and there is a legend
that tells you which wedge corresponds to which category. Just like a line chart,
however, it tends to be easier to read the visualization if each wedge has a nearby
label that indicates what category it is. If the wedges are directly labeled like
this, not only is a legend unnecessarily, but the pie chart might not even need
to use a different color for each wedge. Instead, there could be a simple border
...
inclusiveness_index %>%
  count(Index.categories.2020) %>%
  mutate(Index.categories.2020 = fct_rev(Index.categories.2020),
         cum_n = cumsum(n)) %>%
  ggplot(aes(x=n, y = "", group = Index.categories.2020)) +
    geom_col(fill="#666666", color="#aaaaaa") +
    coord_polar() +
    geom_text(aes(x=cum_n-n/2, y=1.2, label = Index.categories.2020),
              color = "#ffffff") +
    theme_void()
...
inclusiveness_index %>%
  ...
  mutate(Index.categories.2020 = fct_rev(Index.categories.2020),
         cum_n = cumsum(n)) %>%
  ggplot(aes(x=n, y = "", group = Index.categories.2020, 
             fill = Index.categories.2020 == "High")) +
    geom_col(color="#aaaaaa", show.legend = FALSE) +
    coord_polar() +
    geom_text(aes(x=cum_n-n/2, y=1.2, label = Index.categories.2020),
              color = "#ffffff") +
...
The primary variation of a pie chart is a donut chart.

#### Donut chart

A donut chart uses the same data as a pie chart and is organized the same way. In
a donut chart, however, the center is removed, leaving just a ring around the edge
that shows the data. 
...
inclusiveness_index %>%
  ...
  ggplot(aes(x = 4, fill = Index.categories.2020, y=n)) +
    geom_col(position="fill", color="#cccccc") +
...
### Add a variable: Pie chart with color grouping

One way to add a variable inside a pie chart might be to use color. Instead of 
giving each wedge a separate color, the color could correspond to a third variable 
that groups multiple wedges into the same category. For example, a pie chart could
have separate wedges for different countries, and then it could use a different
color for each continent to show how the countries group together into continents.
...
inclusiveness_index %>%
  ...
  mutate(Continent = Continent %>% as_factor() %>% fct_reorder(n, .fun = sum, .desc = TRUE)) %>%
  ggplot(aes(x = 4, fill = Continent, y=n)) +
    geom_col(position="fill", color="grey50") +
    coord_polar(theta="y", direction = -1) +
    #xlim(c(2.5, 4.5)) +
    scale_fill_grey(start = 0.3, end=0.8) +
    theme_void()
...
inclusiveness_index %>% 
    count(Continent, Index.categories.2020) %>%
    mutate(Continent = Continent %>% as_factor() %>% fct_reorder(n, .fun = sum, .desc = TRUE),
           Index.categories.2020 = fct_rev(Index.categories.2020)) %>%
...
inclusiveness_index %>%
  ...
  mutate(index_name = index_name %>%
           ...
           fct_rev()) %>%
  #View() %>%
  ggplot(aes(y = index_name, groups = Index.categories.2020, x = mean_incl_ind, fill = mean_incl_ind)) +
    geom_col() +
...
inclusiveness_index %>%
  ...
  ggplot(aes(y = index_name, groups = Index.categories.2020, x = mean_incl_ind, fill = mean_incl_ind)) +
    geom_col() +
    geom_col(data = . %>% dplyr::filter(Index.categories.2020 == highlight), 
             aes(fill=NULL), color = "#000000", fill=NA) +
    #geom_text(data = . %>% dplyr::filter(Index.categories.2020 == highlight), aes(label=Index.categories.2020),
    #          position = position_stack(vjust = .5)) + 
    #geom_point(data = . %>% dplyr::filter(Index.categories.2020 == highlight), 
    #          position = position_stack(vjust = .5)) + 
    scale_fill_distiller(type="div", palette = 3, 
                         limits=c(-1.2,1.2), 
                         direction = -1) +
...
inclusiveness_index %>%
  ...
  ggplot(aes(y = index_name, x = Index.categories.2020, fill = mean_incl_ind)) +
    ...
    scale_fill_distiller(type="div", palette = 3, 
                         limits=c(-1.2,1.2), 
                         direction = -1) +
    geom_tile(data = . %>% dplyr::filter(Index.categories.2020 == highlight),
              color="#000000", size=.5) +
  theme_bw()
...
ggplot(tibble(x = rnorm(10000000)), aes(x = x))+
  geom_density() +
...
inclusiveness_index %>%
  ...
  scale_x_continuous(limits = c(min(inclusiveness_index$Inclusiveness.index.2020, na.rm = TRUE),
                                max(inclusiveness_index$Inclusiveness.index.2020, na.rm = TRUE)))
...
inclusiveness_index %>%
  ...
  ggplot(aes(x = Continent, y = Inclusiveness.index.2020)) +
    geom_violin() +
  geom_jitter(width = .1, shape = 21, fill = "#cccccc", color = "#000000")
...
inclusiveness_index %>%
  ...
  mutate(index_name = index_name %>%
           ...
           fct_relevel(c("Z.score.General.Population", "Z.score.Race", "Z.score.Gender", 
                         ...
                         "Z.score.Disability", "Inclusiveness.index.2020")) %>%
           fct_rev()) %>%
  ggplot(aes(x = index_name, y = index_values)) +
  geom_violin()
...
inclusiveness_index %>%
  ...
  mutate(index_name = index_name %>%
           ...
           fct_relevel(c("Z.score.General.Population", "Z.score.Race", "Z.score.Gender", 
                         ...
                         "Z.score.Disability", "Inclusiveness.index.2020")) %>%
           fct_rev()) %>%
  ggplot(aes(y = index_name, x = index_values)) +
  geom_violin()
