...
inclusiveness_index_annual %>%
  ...
  summarise(mean_incl_ind = mean(Inclusiveness.index, na.rm = TRUE), .groups = "drop") %>%
...
```{r}
inclusiveness_index_annual %>%
  dplyr::filter(Continent == "Africa", !is.na(Inclusiveness.index)) %>%
  count(Year) %>%
ggplot(aes(x = Year, 
           y = n)) +
  geom_line()
...
#### Matrix / circles with color and size
